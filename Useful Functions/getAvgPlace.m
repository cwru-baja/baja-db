function avgRanks = getAvgPlace(dataIn, wrongNames, years, minComps)
...
for i = 1:size(uniNumbers, 2)
    ...
    for j = 1:size(yearResults, 1)
        ...
        if kkb ~= ""
            if size(kkb, 1) == 1
                for k = 1:size(kkb, 1)
                    pointsTotal = pointsTotal + yearResults{j, 1}.overallRank(yearResults{j, 1}.universityName == kkb(k));
